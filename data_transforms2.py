...
class RandomResizedCrop(object):
    ...
    def get_params(self, img):
        ...
        Args:
            img (list of PIL Image): Images to be cropped.
        Returns:
    ...
    def __call__(self, img, *args):
        """
        Args:
            img (list of PIL Image): Images to be flipped.
        Returns:
        ...
        for k in range(n_size):
            img[k] = resized_crop(img[k], i, j, h, w, self.size, self.interpolation)
        return (img,*args)
...
class RandomCrop(object):
    ...
    def __call__(self, image, label, *args):
        ...
        size_check = True
        for k in range(n_size):
            size_check = (size_check and (image[k].size == label.size) )
        assert label is None or size_check
        ...
        if left > 0 or right > 0 or top > 0 or bottom > 0:
            ...
            for k in range(n_size):
                image[k] = pad_image(
                'reflection', image[k], top, bottom, left, right)
        w, h = image[0].size
        if w == tw and h == th:
        ...
        for k in range(n_size):
            image[k].crop((x1, y1, x1 + tw, y1 + th))
        results = [image]
...
class RandomScale(object):
    ...
    def __call__(self, image, label):
        ...
        n_size = len(image)
        for k  in range(n_size):
            image[k] = image[k].resize((tw, th), interpolation)
        return image, \
...
class RandomRotate(object):
    ...
    def __call__(self, image, label=None, *args):
        ...
        for k in range(n_size):
            image[k] = pad_image('reflection', image[k], h, h, w, w)
            image[k] = image[k].rotate(angle, resample=Image.BILINEAR)
            image[k] = image[k].crop((w, h, w + w, h + h))
        return image, label
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, image, label=None):
        if random.random() < 0.5:
            n_size = len(image)
            for k in range(n_size):
                image[k] = image[k].transpose(Image.FLIP_LEFT_RIGHT)
            if label:
...
class ToTensor(object):
    ...
    def __call__(self, pic, label=None):
        ...
        full_img = []
        for k in range(n_size):
            if isinstance(pic[k], np.ndarray):
                # handle numpy array
                img = torch.from_numpy(pic[k])
            else:
                # handle PIL Image
                img = torch.ByteTensor(
                    torch.ByteStorage.from_buffer(pic[k].tobytes()))
                # PIL image mode: 1, L, P, I, F, RGB, YCbCr, RGBA, CMYK
                if pic[k].mode == 'YCbCr':
                    nchannel = 3
                else:
                    nchannel = len(pic[k].mode)
                img = img.view(pic[k].size[1], pic[k].size[0], nchannel)
            ...
            full_img.append(img)
        # pdb.set_trace()
        full_img = torch.cat(full_img)
        if label is None:
            return (full_img,)
        else:
            return full_img, torch.LongTensor(np.array(label, dtype=np.int))
